        ...
        begin
            if(!reset) begin
                ...
                $display("cycle=%0d pc=%h instr=%h rs1=x%0d(%0d) rs2=x%0d(%0d) rd=x%0d alu=%0d memwrite=%b addr=%0d wdata=%0d",
                         ...
                         dut.Instr[11:7], dut.rvsingle.ALUResult,
                         MemWrite, DataAdr, WriteData);

                if(dut.rvsingle.RegWrite && (dut.Instr[11:7] != 5'd0))
                    $display("  WB:  x%0d <= %0d", dut.Instr[11:7], dut.rvsingle.dp.Result);

                if(MemWrite)
                    $display("  MEM: [%0d] <= %0d", DataAdr, WriteData);

                if(dut.rvsingle.PCSrc)
                    $display("  PC:  next <= %h", dut.rvsingle.dp.PCTarget);
