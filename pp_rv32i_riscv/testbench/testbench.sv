module testbench();
    ...
    function automatic logic [31:0] read_reg(input logic [4:0] regnum);
        read_reg = (regnum == 5'd0) ? 32'd0 : dut.rvpipeline.dp.rf.rf[regnum];
    endfunction
        ...
        begin
            if(!reset) begin
                ...
                $display("cycle=%0d pc=%h instr=%h rs1=x%0d(%0d) rs2=x%0d(%0d) rd=x%0d alu=%0d memwrite=%b addr=%0d wdata=%0d",
                         ...
                         dut.Instr[11:7], dut.rvpipeline.ALUResultM,
                         MemWrite, DataAdr, WriteData);

                if(dut.rvpipeline.RegWriteW && (dut.rvpipeline.dp.RDW != 5'd0))
                    $display("  WB:  x%0d <= %0d", dut.rvpipeline.dp.RDW, dut.rvpipeline.dp.ResultW);

                if(MemWrite)
                    $display("  MEM: [%0d] <= %0d", DataAdr, WriteData);

                if(dut.rvpipeline.PCSrcE)
                    $display("  PC:  next <= %h", dut.rvpipeline.dp.PCTargetE);
